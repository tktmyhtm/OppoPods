...
object MiLinkServiceHook : HookContext() {
    ...
    private var lastGameModeBroadcastMs = 0L
    private val GAME_MODE_BROADCAST_INTERVAL_MS = 3000L
    ...
    private fun throttledBroadcast(action: String, intervalMs: Long, extras: Intent.() -> Unit) {
        ...
        val lastVar = when (action) {
            ...
            OppoPodsAction.ACTION_PODS_ANC_CHANGED -> lastAncBroadcastMs
            OppoPodsAction.ACTION_GAME_MODE_SET -> lastGameModeBroadcastMs
            OppoPodsAction.ACTION_REFRESH_STATUS -> lastStatusRequestMs
            else -> return
        }
        if (now - lastVar < intervalMs) return
        when (action) {
            OppoPodsAction.ACTION_PODS_BATTERY_CHANGED -> lastBatteryBroadcastMs = now
            OppoPodsAction.ACTION_PODS_ANC_CHANGED -> lastAncBroadcastMs = now
            OppoPodsAction.ACTION_GAME_MODE_SET -> lastGameModeBroadcastMs = now
